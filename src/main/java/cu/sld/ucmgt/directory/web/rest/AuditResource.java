...
import cu.sld.ucmgt.directory.service.AuditEventService;
...
public class AuditResource {
    ...
    public ResponseEntity<List<AuditEvent>> getAll(Pageable pageable) {
        ...
        HttpHeaders headers = PaginationUtil.generatePaginationHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
 ...
 */
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the {@link AuditEvent} in body,
     * or status {@code 404 (Not Found)}.
     */
    @GetMapping("/{id:.+}")
    public ResponseEntity<AuditEvent> get(@PathVariable UUID id) {
        return ResponseUtil.wrapOrNotFound(auditEventService.findById(id));
    }
}
